...
void approachMissionSite(){
  ...
  otv.moveToUntilObstacle({otv.getPosition().x-0.09, (startAtA ? 0.25 : 1.75)}); //moves to: (PA if MB, PB if MA)
  //otv.moveToUntilObstacle({otv.getPosition().x-0.0381, (startAtA ? 0.25 : 1.75)});
  otv.stop();
}
